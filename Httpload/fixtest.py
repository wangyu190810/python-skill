...
PORT = 8888 #端口
URI = "/api/trade/stock/entrust" #相对地址,加参数防止缓存，否则可能会返回304
TOTAL = 0 #总数
SUCC = 0 #响应成功数
FAIL = 0 #响应失败数
EXCEPT = 0 #响应异常数
...
class RequestThread(threading.Thread):
    # 构造函数
    def __init__(self, thread_name, url,post_data):
    ...
    def test_performace(self):
        ...
        try:
           ...
           # conn.request('POST', URI, self.post_data)
            res = requests.post(sel.url,self.post_data)
...
if __name__ == "__main__":
    ...
    post_datas = [{"uid":100111,"stock_code":601991,"entrust_amount":100,"entrust_price":5.62,"entrust_direction":0},
                    {"uid":100111,"stock_code":601991,"entrust_amount":50,"entrust_price":5.62,"entrust_direction":0},
                    {"uid":100111,"stock_code":601991,"entrust_amount":20,"entrust_price":5.62,"entrust_direction":0},
                    {"uid":100111,"stock_code":601991,"entrust_amount":10,"entrust_price":5.62,"entrust_direction":0}]

    test(10, post_datas[2])
    test(20, str(post_datas[2]))
    test(50, str(post_datas[2]))
    test(100, str(post_datas[2]))
    test(500, str(post_datas[2]))
    test(1000, str(post_datas[2]))
    test(2000, str(post_datas[2]))
    test(3000, str(post_datas[2]))
    test(4000, str(post_datas[2]))
    test(5000, str(post_datas[2]))
    test(6000, str(post_datas[2]))
    test(7000, str(post_datas[2]))
    test(8000, str(post_datas[2]))
    test(9000, str(post_datas[2]))
    test(10000, str(post_datas[2]))
    test(10, str(post_datas[3]))
    test(20, str(post_datas[3]))
    test(50, str(post_datas[3]))
    test(100, str(post_datas[3]))
